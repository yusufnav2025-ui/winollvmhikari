#include "Obfuscation/MBAMatrix.h"
#include <algorithm>
#include <cstdint>
#include <cstdlib>
#include <map>
#include <vector>
namespace polaris {
// Bare abs() from <cstdlib> is int abs(int) -- it will silently truncate
// int64_t coefficients on some standard library implementations/inclusion
// orders. Use an explicit 64-bit-safe helper everywhere instead.
static inline int64_t abs64(int64_t V) { return V < 0 ? -V : V; }
void MBAMatrix::simplify() {
  for (int i = 0; i < Line; i++) {
    int64_t Gcd = 0;
    for (int j = 0; j < Column; j++) {
      int64_t Val = abs64(getElement(i, j));
      if (Val == 0) {
        continue;
      }
      if (Gcd == 0) {
...
void MBAMatrix::lineElimate(int Dst, int Src, int ColumnIdx) {
  ...
  bool IsAdd = (A > 0) ^ (B > 0);
  int64_t FactorA = lcm(abs64(A), abs64(B)) / abs64(A),
          FactorB = lcm(abs64(A), abs64(B)) / abs64(B);
...
void MBAMatrix::solve(std::vector<int64_t> &Solution) {
  ...
  for (int i = Rank - 1; i >= 0; i--) {
    ...
    int64_t Factor = abs64(getElement(i, VarIdx));
